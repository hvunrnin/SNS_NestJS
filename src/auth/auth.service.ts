...
export class AuthService {
     ...
     *    예를 들어 현재 로그인한 사용자가 작성한 포스트만 가져오려면 토큰의 sub값에 입력되어있는 사용자의 포스트만 따로 필터링 가능
     *    특정 사용자의 토큰이 없다면 다른 사용자의 데이터 접근 불가
     * 5) 모든 토큰은 만료 기간이 있음. 만료기간이 지나면 새로 토큰을 발급받아야함
     ...
     * 6) 토큰이 만료되면 각각의 토큰을 새로 발급 받을 수 있는 엔드포인트에 요청을 해서 새로운 토큰을 발급받고 새로운 토큰을 사용해 pricate route에 접근 (다시 4번) 
    */
     ...
     *        회원가입 후 다시 로그인해주세요 <- 이런 쓸데없는 과정 방지 위해
     * 
     * 2) loginWithEmail
    ...
    */
    ...
    /**
     * Basic sjakljslkgjlkjfsklfd
     * 
     * 1) sjakljslkgjlkjfsklfd -> email:password
    ...
    rotateToken(token: string, isRefreshToken: boolean){
        ...
        */
        ...
        return this.signToken({
            ...decoded,
        }, isRefreshToken); // isRefreshToken이 true면 refreshToken 발급, false면 accessToken 발급
    }
